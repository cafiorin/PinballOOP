...
#define LONGDELAY 200  // This delay BETWEEN demos
...
//branco, verde,roxo,vermelho
/* Arduino
static const unsigned char ht1632_data = 12;  // Data pin (pin 7 of display connector) - fio roxo
static const unsigned char ht1632_wrclk = 13; // Write clock pin (pin 5 of display connector) - fio vermelho
static const unsigned char ht1632_cs = 10;    // Chip Select (pin 1 of display connnector) - fio branco
static const unsigned char ht1632_clk = 11; // clock pin (pin 2 of display connector) - fio verde
*/

static const unsigned char ht1632_data = 46;  // Data pin (pin 7 of display connector) - fio roxo
static const unsigned char ht1632_wrclk = 48; // Write clock pin (pin 5 of display connector) - fio vermelho
static const unsigned char ht1632_cs = 42;    // Chip Select (pin 1 of display connnector) - fio branco
static const unsigned char ht1632_clk = 44; // clock pin (pin 2 of display connector) - fio verde

// our own copy of the "video" memory; 64 unsigned chars for each of the 4 screen quarters;
...
unsigned char ht1632_shadowram[64][4 * 2] = {
	0 };
...
//**************************************************************************************************
byte get_pixel(unsigned char x, unsigned char y) {
...
/*
* plot a pobyte on the display, with the upper left hand corner
...
*/
void ht1632_plot(byte x, byte y, unsigned char color)
{
...
void ht1632_putchar(unsigned char x, unsigned char y, char c, unsigned char color = GREEN)
{
	unsigned char dots;
...
#ifdef FONT_4x6
	if (c >= 'A' && c <= 'Z') {
		c = c - 'A' + 33;   // A-Z maps to 1-26
	}
	else if (c >= 'a' && c <= 'z') {
		c = c - 'a' + 65;   // A-Z maps to 1-26
	}
	else if (c >= '0' && c <= '9') {
		c = (c - '0') + 16;
	}
	...
	for (char col = 0; col < FONT_COL; col++) {
...
void ht1632_putchar1(unsigned char x, unsigned char y, char c, unsigned char color = GREEN, unsigned char font = 0)
{
	unsigned char dots;
	...
	for (char col = 0; col < (font == 0 ? FONT_COL : FONT_COL1); col++)
	{
		if (font == 1)
		{
			dots = pgm_read_byte_near(&font1[c][col]);
		}
		else
		{
			dots = pgm_read_byte_near(&myfont[c][col]);
...
*/
void ht1632_putcharsizecolor(byte x, byte y, unsigned char c, char size, unsigned char color, unsigned char secondcolor, unsigned char fontname [][NCOLUMNS], byte columncountfont, char rowcountfont, char oddeven)
{

	unsigned char dots, dots2, dots3, cc, cc2, cc3, rr, g, t, t3, divisor;
	unsigned char maximumdrawfont, showcolor, showsecondcolor; //128 for the large fonts (=8x8 and 12x8), 64 for all smaller ones
	if (rowcountfont <= 7)
		maximumdrawfont = 64;
	else
		maximumdrawfont = 128;
	...
	for (unsigned char col = 0; col < columncountfont*size; col++) {
		...
		for (unsigned char row = 0; row < rowcountfont / divisor*size; row++) {
			if (color == 5){
				showcolor = random(3) + 1;
			}
			else
...
void ht1632_putcharsize1D(byte x, byte y, unsigned char c, char size, unsigned char color, unsigned char secondcolor, unsigned char * fontname, char columncountfont, char rowcountfont, char oddeven)
{
...
void ht1632_putbigbitmap(byte x, byte y, unsigned char color, unsigned char secondcolor, unsigned char * bitmapname, byte columncountbitmap, unsigned char rowcountbitmap, char oddeven)
{
	...
	unsigned char maximumdrawbitmap = 128, showcolor, showsecondcolor; //128 for the large fonts (=8x8 and 12x8), 64 for all smaller ones
	if (x >= 0){
		startcolum = 0;
	}
	else{
	...
	for (unsigned short col = startcolum; col < endcolumn; col++) {
		//for (unsigned char col=0; col<columncountbitmap; col++) {
		// Addressing the right 8 lines because 'The Dot Factory' starts from the bottom, all others from top
		if (rowcountbitmap <= 8) {
			cc = col;
		...
		for (unsigned char row = 0; row < rowcountbitmap / divisor; row++) {
...
void scrollbitmapxcolor(byte y, unsigned char color, unsigned char secondcolor, unsigned char * bitmapname, byte columncountbitmap, unsigned char rowcountbitmap, char oddeven, byte delaytime) {
...
*/
void scrollbitmapycolor(byte x, unsigned char color, unsigned char secondcolor, unsigned char * bitmapname, byte columncountbitmap, unsigned char rowcountbitmap, char oddeven, byte delaytime) {
	unsigned char showcolor, showsecondcolor;
	byte ya = 0;
	while (ya < 1) {
		byte ypos = Y_MAX;
...
void scrolltextxcolor(byte y, char Str1 [], unsigned char color, byte delaytime){
	byte messageLength = strlen(Str1) + 1;
...
*/
void scrolltextsizexcolor(byte y, char Str1 [], char size, unsigned char color, unsigned char secondcolor, unsigned char fontname [][NCOLUMNS], byte columncountfont, char rowcountfont, char oddeven, byte delaytime){
	byte messageLength = strlen(Str1) + 1;
	unsigned char showcolor, showsecondcolor;
	byte xa = 0;
	while (xa < 1) {
...
*/
void scrolltextsizey(byte x, char Str1 [], char size, unsigned char color, unsigned char secondcolor, unsigned char fontname [][NCOLUMNS], byte columncountfont, char rowcountfont, char oddeven, byte delaytime){
	byte messageLength = strlen(Str1) + 1;
	unsigned char showcolor, showsecondcolor;
	byte ya = 0;
	while (ya < 1) {
...
*/
void scrolltextsize1Dxcolor(byte y, char Str1 [], char size, unsigned char color, unsigned char secondcolor, unsigned char * fontname, byte columncountfont, char rowcountfont, char oddeven, byte delaytime){
	byte messageLength = strlen(Str1) + 1;
	unsigned char showcolor, showsecondcolor;
	byte xa = 0;
	while (xa < 1) {
...
void textcolor(byte x, byte y, char Str1 [], unsigned char color)
{
	byte messageLength = strlen(Str1) + 1;
	unsigned char showcolor;
	for (byte i = 0; i < messageLength; i++)
	{
		...
		ht1632_putchar(x + (i*NCOLUMNS), y, Str1[i], showcolor);
	}

}

void textcolor1(byte x, byte y, char Str1 [], unsigned char color, char cfont)
{
	byte messageLength = strlen(Str1) + 1;
	unsigned char showcolor;
	for (byte i = 0; i < messageLength; i++)
	{
		...
		if (cfont == 1)
			ht1632_putchar1(x + (i*NCOLUMNS1), y, Str1[i], showcolor,cfont);
		else
			ht1632_putchar1(x + (i*NCOLUMNS), y, Str1[i], showcolor,cfont);
